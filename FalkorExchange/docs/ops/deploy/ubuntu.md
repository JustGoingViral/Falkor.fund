...
Insert the following lines into the bitcoin.conf, and replce with your username and password.
    ...
    walletnotify=/usr/local/sbin/rabbitmqadmin publish routing_key=falkor.deposit.coin payload='{"txid":"%s", "channel_key":"satoshi"}'
...
##### Clone the Source
    ...
    mkdir -p ~/falkor
    git clone https://github.com/rubykube/falkor.git ~/falkor/current
    cd falkor/current
...
Read how to deal with Falkor Exchange daemons at [Falkor Exchange daemons](https://github.com/rubykube/falkor/blob/master/docs/falkor/daemons.md).
...
**Passenger:**
    ...
    sudo ln -s /home/deploy/falkor/current/config/nginx.conf /etc/nginx/conf.d/falkor.conf
