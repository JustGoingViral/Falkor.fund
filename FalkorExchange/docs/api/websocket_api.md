# Falkor Exchange WebSocket API

Falkor Exchange WebSocket API connections are handled by Ranger service provided by
[falkor gem](https://github.com/rubykube/falkor-core).
...
**Note:** Falkor Exchange websocket API supports authentication only Bearer type of JWT token.
...
| ------------------ | ------------------------------------------------------------ |
| `id`               | Unique order id.                                             |
| `market`           | The market in which the order is placed. (In falkor `market_id`) |
...
| `at`               | Order create time. (Deprecated) (In falkor `created_at`)     |
...
| `market`     | The market in which the trade is placed. (In falkor market_id) |
...
Start ranger websocket server using following command in falkor-core gem:

```bash
$ ./bin/falkor service start ranger
```
