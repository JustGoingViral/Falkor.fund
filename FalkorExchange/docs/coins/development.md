...
Falkor Exchange Plugin API v2 gives ability to extend Falkor Exchange with any coin
which fits into basic [Blockchain](https://www.rubydoc.info/gems/falkor/0.5.0/Falkor Exchange/Blockchain/Abstract) and [Wallet](https://www.rubydoc.info/gems/falkor/0.5.0/Falkor Exchange/Blockchain/Abstract)
interfaces described inside [falkor-core](https://github.com/rubykube/falkor-core) gem.

## Development.

### Start from reading Blockchain and Wallet doc.

You need to be familiar with [Blockchain](https://www.rubydoc.info/gems/falkor/0.5.0/Falkor Exchange/Blockchain/Abstract)
and [Wallet](https://www.rubydoc.info/gems/falkor/0.5.0/Falkor Exchange/Blockchain/Abstract) interfaces.
...
You need to study your coin API to get list of calls for implementing [Blockchain](https://www.rubydoc.info/gems/falkor/0.5.0/Falkor Exchange/Blockchain/Abstract) and
[Wallet](https://www.rubydoc.info/gems/falkor/0.5.0/Falkor Exchange/Blockchain/Abstract) interfaces. 
...
We will use [falkor-litecoin](https://github.com/rubukybe/falkor-litecoin) as example.
My advice is to clone it and use as plugin development guide.

For more currencies examples check [Bitcoin](../../lib/falkor/bitcoin) and [Ethereum](../../lib/falkor/ethereum) implementation.

1. ***Create a new gem. And update .gemspec.*** 💎

```bash
bundle gem falkor-litecoin
```
...
```ruby
  ...
  spec.add_dependency "falkor", "~> 0.6.1"          # Required.
...
Falkor Exchange-litecoin spec/resources directory has the following structure:
...
Falkor Exchange-litecoin has the following lib and spec structure:
...
```ruby
client = Falkor Exchange::Litecoin::Client.new('http://user:password@127.0.0.1:19332') # => #<Falkor Exchange::Litecoin::Client:0x00007fca61d82650 @json_rpc_endpoint=#<URI::HTTP http://user:password@127.0.0.1:19332>>
...
E.g. specs for falkor-litecoin client:

```bash
bundle exec rspec spec/falkor/litecoin/client_spec.rb

Falkor Exchange::Litecoin::Client
  initialize
  ...
  json_rpc
    getblockcount
      ...
      should eq 40500
    methodnotfound
      should raise Falkor Exchange::Litecoin::Client::ResponseError with "Method not found (-32601)"
    notfound
      should raise Falkor Exchange::Litecoin::Client::Error
    connectionerror
      should raise Falkor Exchange::Litecoin::Client::ConnectionError
...
module Falkor Exchange
  module Litecoin
    class Blockchain < Falkor Exchange::Blockchain::Abstract
...
E.g. specs for falkor-litecoin blockchain:

```bash
Falkor Exchange::Litecoin::Blockchain
  features
    defaults
    override defaults
    custom feautures
...
module Falkor Exchange
  module Litecoin
    class Wallet < Falkor Exchange::Blockchain::Abstract
    # Your custom logic goes here.
    end
  end
end
...
E.g. specs for falkor-litecoin wallet:

```bash
Falkor Exchange::Litecoin::Wallet
  configure
    requires wallet
    requires currency
...
13. ***Register your plugin blockchain and wallet to make it accessible by Falkor Exchange.️*** ®

```ruby
Falkor Exchange::Blockchain.registry[:litecoin] = Litecoin::Blockchain.new
Falkor Exchange::Wallet.registry[:litecoind] = Litecoin::Wallet.new
```
...
14. ***Test your plugin inside falkor eco system.*** 🧪
...
* integration.md
    ...
    **Image Build** and **Falkor Exchange Configuration** sections are required.
...
For doing it left comment with your plugin link and short description [here](https://github.com/rubykube/falkor/issues/2212).
