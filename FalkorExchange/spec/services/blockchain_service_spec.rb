...
describe BlockchainService do
  ...
  let(:transaction) { Falkor Exchange::Transaction.new(hash: 'fake_txid', to_address: 'fake_address', from_addresses: ['fake_address'], amount: 5, block_number: 3, currency_id: 'fake1', txout: 4, status: 'success') }
  ...
  let(:expected_transactions) do
    ...
    ].map { |t| Falkor Exchange::Transaction.new(t) }
  end

  let(:expected_block) { Falkor Exchange::Block.new(block_number, expected_transactions) }

  before do
    wallet.currencies << [fake_currency1, fake_currency2]
    Falkor Exchange::Blockchain.registry.expects(:[])
                         .with(:fake)
  ...
  describe 'Filter Deposits' do
    ...
    context 'single fake deposit was created during block processing' do
      ...
      context 'creates deposit with correct attributes' do
        before do
          service.adapter.stubs(:fetch_block!).returns(Falkor Exchange::Block.new(block_number, [transaction]))
          service.process_block(block_number)
        end
  ...
  context 'two fake withdrawals for two currency were updated during block processing' do
    ...
    context 'fail withdrawal if transaction has status :fail' do
      ...
      let!(:transaction) do
        Falkor Exchange::Transaction.new(hash: 'fake_hash', to_address: 'fake_address', amount: 1, block_number: 3, currency_id: fake_currency1.id, txout: 10, status: 'failed')
      end
    ...
    context 'fail withdrawal if transaction has status :fail' do
      ...
      let!(:transaction) do
        Falkor Exchange::Transaction.new(hash: 'fake_hash', to_address: 'fake_address', amount: 1, block_number: 3, currency_id: fake_currency1.id, txout: 10, status: 'pending')
      end
    ...
    context 'succeed withdrawal if transaction has status :success' do
      ...
      let!(:succeed_transaction) do
        Falkor Exchange::Transaction.new(hash: 'fake_hash', to_address: 'fake_address', amount: 1, block_number: 3, currency_id: fake_currency1.id, txout: 10, status: 'success')
      end
  ...
  describe 'Several blocks' do
    ...
    let(:expected_block1) { Falkor Exchange::Block.new(block_number, expected_transactions1) }
