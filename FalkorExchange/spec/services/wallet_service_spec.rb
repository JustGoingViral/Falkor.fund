...
describe WalletService do
  ...
  before do
    ...
    Falkor Exchange::Wallet.registry.expects(:[])
                     .with(:fake)
  ...
  context :build_withdrawal! do
    ...
    let(:transaction) do
      Falkor Exchange::Transaction.new(hash:        '0xfake',
    ...
    context 'Single wallet' do
      ...
      context 'single wallet available' do
        ...
        it 'spreads everything to single wallet' do
          ...
          expect(subject).to all(be_a(Falkor Exchange::Transaction))
        end
      end
  ...
  context :collect_deposit do
    ...
    context 'Spread deposit with single entry' do
      ...
      let(:spread_deposit) do
        [Falkor Exchange::Transaction.new(to_address: 'fake-cold',
                                 amount: '2.0',
                                 currency_id: currency.id)]
      end
    ...
    context 'Spread deposit with two entry' do
      ...
      let(:spread_deposit) do
        [Falkor Exchange::Transaction.new(to_address: 'fake-hot',
                                amount: '2.0',
                                currency_id: currency.id),
         Falkor Exchange::Transaction.new(to_address: 'fake-hot',
                                 amount: '2.0',
      ...
      let(:transaction) do
        [{ hash:        '0xfake',
         ...
         { hash:        '0xfake1',
           to_address:  cold_wallet.address,
           amount:      deposit.amount,
           currency_id: currency.id }].map { |t| Falkor Exchange::Transaction.new(t)}
      end
  ...
  context :refund do
    ...
    it 'creates single transaction' do
      ...
      expect(subject).to be_a(Falkor Exchange::Transaction)
    end
  end
end
