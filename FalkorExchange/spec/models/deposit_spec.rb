...
describe Deposit do
  ...
  context :spread_between_wallets! do
    let(:spread) do
      [Falkor Exchange::Transaction.new(to_address: 'to-address-1', amount: 1.2),
       Falkor Exchange::Transaction.new(to_address: 'to-address-2', amount: 2.5)]
    end
    ...
    context :record_complete_operations! do
      ...
      context 'coin deposit' do
        ...
        context 'with manual deposit approval' do
          before { Falkor Exchange::App.config.stubs(:manual_deposit_approval).returns(true) }
          context 'credits both legacy and operations based member balance for coin deposit' do
            subject { create(:deposit_btc, amount: 3.7) }
        ...
        context 'with locked funds' do
          before { Falkor Exchange::App.config.stubs(:deposit_funds_locked).returns(true) }
          context 'credits both legacy and operations based member balance for coin deposit' do
            subject { create(:deposit_btc, amount: 3.7) }
  ...
  context :process do
    ...
    context 'coin deposit' do
      ...
      context 'manual process' do
        before do
          Falkor Exchange::App.config.stubs(:manual_deposit_approval).returns(true)
        end
  ...
  context 'process collect' do
    ...
    context 'with manual deposit approval' do
      ...
      context 'deposit_funds_locked enabled' do
        before do
          Falkor Exchange::App.config.stubs(:deposit_funds_locked).returns(true)
          crypto_deposit.accept!
          crypto_deposit.process!
        end
