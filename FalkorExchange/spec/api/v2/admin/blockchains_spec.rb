...
describe API::V2::Admin::Blockchains, type: :request do
  ...
  describe 'POST /api/v2/admin/blockchains/process_block' do
    ...
    context 'successful' do
      ...
      context 'deposit' do
        let(:transaction) { Falkor Exchange::Transaction.new(hash: 'fake_txid', from_addresses: ['fake_address'], to_address: 'fake_address', amount: 5, block_number: block_number, currency_id: 'fake', txout: 4, status: 'success') }
        let(:expected_block) { Falkor Exchange::Block.new(block_number, [transaction]) }
      ...
      context 'withdraw' do
        ...
        let!(:transaction) do
          Falkor Exchange::Transaction.new(hash: 'fake_hash', to_address: 'fake_address', amount: 1, block_number: block_number, currency_id: currency.id, txout: 10, status: 'pending')
        end

        let!(:succeed_transaction) do
          Falkor Exchange::Transaction.new(hash: 'fake_hash', to_address: 'fake_address', from_addresses: ['fake_address'], amount: 1, block_number: block_number, currency_id: currency.id, txout: 10, status: 'success')
        end
