...
describe API::V2::Management::Orders, type: :request do
  ...
  describe 'POST /api/v2/management/orders/:id/cancel' do
    let!(:falkor_order) { create(:order_ask, :btcusd, member: member1, state: Order::WAIT) }
    ...
    it 'cancels an order on falkor market' do
      AMQP::Queue.expects(:enqueue).with(:matching, action: 'cancel', order: falkor_order.to_matching_attributes)
      AMQP::Queue.expects(:publish).with(finex_engine.driver, data: falkor_order.as_json_for_third_party, type: 3).never
      request(falkor_order.id)
      expect(response).to have_http_status 200
    end
  ...
  describe 'POST /api/v2/management/orders/cancel' do
    let!(:member1_falkor_order) {  create(:order_ask, :btcusd, member: member1, state: Order::WAIT) }
    let!(:member2_falkor_order) {  create(:order_ask, :btcusd, member: member2, state: Order::WAIT) }
    ...
    context 'falkor order cancel' do

      it 'cancels the orders on falkor market' do
        data[:market] = 'btcusd'

        AMQP::Queue.expects(:enqueue).with(:matching, action: 'cancel', order: member1_falkor_order.to_matching_attributes)
        AMQP::Queue.expects(:enqueue).with(:matching, action: 'cancel', order: member2_falkor_order.to_matching_attributes)
    ...
    context 'third party orders cancel' do
      it 'cancels the orders on third party market' do
        ...
        AMQP::Queue.expects(:enqueue).with(:matching, action: 'cancel', order: member1_falkor_order.to_matching_attributes).never
        AMQP::Queue.expects(:enqueue).with(:matching, action: 'cancel', order: member2_falkor_order.to_matching_attributes).never
