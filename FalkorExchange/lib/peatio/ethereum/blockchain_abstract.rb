module Ethereum
  class BlockchainAbstract < Falkor Exchange::Blockchain::Abstract
    ...
    def fetch_block!(block_number)
      ...
      if block_json.blank? || block_json['transactions'].blank?
        return Falkor Exchange::Block.new(block_number, [])
      end
      block_json.fetch('transactions').each_with_object([]) do |tx, block_arr|
        ...
        txs = build_transactions(tx).map do |ntx|
          Falkor Exchange::Transaction.new(ntx)
        end

        block_arr.append(*txs)
      end.yield_self { |block_arr| Falkor Exchange::Block.new(block_number, block_arr) }
    ...
    def load_balance_of_address!(address, currency_id)
      ...
      if currency.dig(:options, contract_address_option).present?
        ...
        raise Falkor Exchange::Blockchain::ClientError.new("Currency #{currency_id} doesn't have option #{contract_address_option}")
      end
    rescue Ethereum::Client::Error => e
      raise Falkor Exchange::Blockchain::ClientError, e
    end
