module Ethereum
  class WalletAbstract < Falkor Exchange::Wallet::Abstract
    ...
    def create_transaction!(transaction, options = {})
      if @currency.dig(:options, contract_address_option).present?
        ...
        raise Falkor Exchange::Wallet::ClientError.new("Currency #{@currency[:id]} doesn't have option #{contract_address_option}")
      end
    rescue Ethereum::Client::Error => e
      raise Falkor Exchange::Wallet::ClientError, e
    end
    ...
    def convert_to_base_unit(value)
      ...
      unless (x % 1).zero?
        raise Falkor Exchange::Wallet::ClientError,
