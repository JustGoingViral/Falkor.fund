module Bitcoin
  ...
  class Blockchain < Falkor Exchange::Blockchain::Abstract
      ...
      client.json_rpc(:getblock, [block_hash, 2])
        .fetch('tx').each_with_object([]) do |tx, txs_array|
          txs = build_transaction(tx).map do |ntx|
            Falkor Exchange::Transaction.new(ntx.merge(block_number: block_number))
          end
          txs_array.append(*txs)
        end.yield_self { |txs_array| Falkor Exchange::Block.new(block_number, txs_array) }
    rescue Bitcoin::Client::Error => e
      raise Falkor Exchange::Blockchain::ClientError, e
    end
    ...
    def fetch_transaction(transaction)
      ...
      if tx.present?
        Falkor Exchange::Transaction.new(tx)
      else
        Falkor Exchange::Transaction.new
      end
    end
    ...
    def load_balance_of_address!(address, _currency_id)
      ...
      if address_with_balance.blank?
        raise Falkor Exchange::Blockchain::UnavailableAddressBalanceError, address
      end
    ...
    def settings_fetch(key)
      @settings.fetch(key) { raise Falkor Exchange::Blockchain::MissingSettingError, key.to_s }
    end
  end
end
