...
require 'csv'
require 'falkor/import'

namespace :import do
  # Detailed instruction https://github.com/rubykube/falkor/blob/master/docs/tasks/import.md
  ...
  desc 'Load addresses from csv file. Export file from Falkor Exchange version >= 2.6.0'
  ...
  task :configs, [:config_load_path] => :environment do |_, args|
    ...
    Falkor Exchange::Import.new(import_data).load_all
  end
  ...
  task :trade_to_influx, [:full_load] => :environment do |_, args|
    ...
    if args.full_load == 'false'
      ids = []
      Falkor Exchange::InfluxDB.config[:host].each do |host|
        client = Falkor Exchange::InfluxDB.client(host: [host])
  ...
  def process_trades_batch(batch)
    batch.each_with_index do |trade, index|
      ...
      Falkor Exchange::InfluxDB.client(keyshard: trade.market_id).write_point('trades', influx_data, "ms")
    end
  end
