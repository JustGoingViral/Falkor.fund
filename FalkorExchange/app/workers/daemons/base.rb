  ...
  module Daemons
    class Base
      ...
      def lock(klass, timeout)
        res = ActiveRecord::Base.connection.exec_query("SELECT GET_LOCK('Falkor Exchange_#{klass}',#{timeout})")
        ...
        if res.to_a[0].values[0] == 1
          begin
            ...
            ActiveRecord::Base.connection.exec_query("SELECT RELEASE_LOCK('Falkor Exchange_#{klass}')")
          end
        else
          raise GetLockError, "Falkor Exchange_#{klass} is already running"
        end
      end
    end
  end
