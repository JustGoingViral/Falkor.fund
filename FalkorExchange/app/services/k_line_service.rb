...
class KLineService
  ...
  def get_ohlc(options = {})
    ...
    Falkor Exchange::InfluxDB.client(keyshard: @market_id, epoch: 's').query(q.join(' ')) do |_name, _tags, points|
  ...
  def calculate_offset(options)
    ...
    Falkor Exchange::InfluxDB.client(keyshard: @market_id, epoch: 's').query(q.join(' ')) do |_, _, values|
