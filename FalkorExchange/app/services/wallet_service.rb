class WalletService
  ...
  def initialize(wallet)
    @wallet = wallet
    @adapter = Falkor Exchange::Wallet.registry[wallet.gateway.to_sym].new(wallet.settings.symbolize_keys)
  end
  ...
  def build_withdrawal!(withdrawal)
    ...
    transaction = Falkor Exchange::Transaction.new(to_address: withdrawal.rid,
  ...
  def deposit_collection_fees!(deposit, deposit_spread)
    ...
    deposit_transaction = Falkor Exchange::Transaction.new(hash:         deposit.txid,
                                                  txout:        deposit.txout,
  ...
  def refund!(refund)
    ...
    refund_transaction = Falkor Exchange::Transaction.new(to_address: refund.address,
  ...
  rescue Falkor Exchange::Wallet::Error => e
    report_exception(e)
  ...
  # @return [Array<Falkor Exchange::Transaction>] result of spread in form of
  ...
  def spread_between_wallets(deposit, destination_wallets)
    ...
    spread = destination_wallets.map do |dw|
      ...
      transaction = Falkor Exchange::Transaction.new(transaction_params)
