...
class Member < ApplicationRecord
  ...
  class << self
    ...
    def validate_payload(p)
      fetch_email(p)
      p.fetch(:uid).tap { |uid| raise(Falkor Exchange::Auth::Error, 'UID is blank.') if uid.blank? }
      p.fetch(:role).tap { |role| raise(Falkor Exchange::Auth::Error, 'Role is blank.') if role.blank? }
      p.fetch(:level).tap { |level| raise(Falkor Exchange::Auth::Error, 'Level is blank.') if level.blank? }
      p.fetch(:state).tap do |state|
        raise(Falkor Exchange::Auth::Error, 'State is blank.') if state.blank?
        raise(Falkor Exchange::Auth::Error, 'State is not active.') unless state == 'active'
      end
    end

    def fetch_email(payload)
      payload[:email].to_s.tap do |email|
        raise(Falkor Exchange::Auth::Error, 'E-Mail is blank.') if email.blank?
        raise(Falkor Exchange::Auth::Error, 'E-Mail is invalid.') unless EmailValidator.valid?(email)
      end
    end
