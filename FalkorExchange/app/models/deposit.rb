...
class Deposit < ApplicationRecord
  ...
  aasm whiny_transitions: false do
    ...
    event :accept do
      ...
      after do
        if currency.coin? && (Falkor Exchange::App.config.deposit_funds_locked ||
                              Falkor Exchange::AML.adapter.present? ||
                              Falkor Exchange::App.config.manual_deposit_approval)
    ...
    event :process do
      transitions from: %i[aml_processing aml_suspicious accepted errored], to: :aml_processing do
        guard do
          Falkor Exchange::AML.adapter.present? || Falkor Exchange::App.config.manual_deposit_approval
        end
    ...
    event :process_collect do
      transitions from: %i[aml_processing aml_suspicious], to: :processing do
        guard do
          currency.coin? && (Falkor Exchange::AML.adapter.present? || Falkor Exchange::App.config.manual_deposit_approval)
        end

        after do
          if !Falkor Exchange::App.config.deposit_funds_locked
    ...
    event :aml_suspicious do
      transitions from: :aml_processing, to: :aml_suspicious do
        guard { Falkor Exchange::AML.adapter.present? || Falkor Exchange::App.config.manual_deposit_approval  }
      end
    end
  ...
  def aml_check!
    ...
    return nil if Falkor Exchange::App.config.manual_deposit_approval && Falkor Exchange::AML.adapter.blank?

    from_addresses.each do |address|
      result = Falkor Exchange::AML.check!(address, currency_id, member.uid)
  ...
  def spread_to_transactions
    spread.map { |s| Falkor Exchange::Transaction.new(s) }
  end
  ...
  def record_submit_operations!
    transaction do
      ...
      locked_kind_check = currency.coin? && (Falkor Exchange::App.config.deposit_funds_locked || Falkor Exchange::App.config.manual_deposit_approval)
