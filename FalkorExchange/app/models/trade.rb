...
class Trade < ApplicationRecord
  ...
  class << self
    ...
    def public_from_influx(market, limit = 100, options = {})
      ...
      Falkor Exchange::InfluxDB.client(keyshard: market).query trades_query.join(' '), params: options.merge(market: market) do |_name, _tags, points|
    ...
    def market_ticker_from_influx(market)
      ...
      Falkor Exchange::InfluxDB.client(keyshard: market).query tickers_query, params: { market: market } do |_name, _tags, points|
    ...
    def trade_from_influx_before_date(market, date)
      ...
      Falkor Exchange::InfluxDB.client(keyshard: market).query trades_query, params: { market: market, date: date.to_i } do |_name, _tags, points|
        return points.map(&:deep_symbolize_keys!).first
      end
    end
  ...
  def write_to_influx
    Falkor Exchange::InfluxDB.client(keyshard: market_id).write_point(self.class.table_name, influx_data, "ns")
  end
